...
public class ManageProxyMSController {
	...
	public List<ManageProxyDTO> getProxyDef(@RequestHeader("Authorization") String authorizationToken){
    	...
    	public ManageProxyDTO[] getProxyDef(@RequestHeader("Authorization") String authorizationToken){
        	ManageProxyDTO[] proxyDefList=	manageProxyService.getProxyDef(authorizationToken);
    		return proxyDefList;
    		
    	}

        @GetMapping("/proxydef/{proxyDefId}") //List given product
	public  List<ManageProxyDTO>  getProxyDefById(@RequestHeader("Authorization") String authorizationToken,@PathVariable int proxyDefId) throws NODataFoundException{
        	List<ManageProxyDTO> pDTO = manageProxyService.getProxyDefById(authorizationToken,proxyDefId);
		 return pDTO;
        }
